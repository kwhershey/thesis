...
where $\tau$ is the natural exciton lifetime, determined by the radiative ($k_r$) and non-radiative ($k_{nr}$) decay rates by $\tau=1/(k_r+k_{nr})$, $k_{TT}$ is the rate constant for triplet-triplet annihilation, $k_{TP}$ is the rate constant for triplet-polaron quenching, and $G_{ex}$ is the exciton generation rate.  
...
where $k_F$ is the rate constant for exciton formation.  The factor of four accounts for the diversity of the polaron population and assumes that electrons and holes are in equal proportion.  
